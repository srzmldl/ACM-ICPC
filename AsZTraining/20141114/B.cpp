...
arrayN fir, f, point, oneNum, slack, pre, prec;
arrayM e, nxt, c, vis, pos;

int hasTot[10][10], has[10][10][10000][10];
...
void solve()
{
    ...
    REPP(i, 1, sTot)
    {
...

		{
			...
			REPP(i, 2, pTot)
			{
                for (int tmp = point[i]; tmp != point[1]; tmp = pre[tmp])
					if (vis[prec[tmp]] == clk) break;
					else
					{
						vis[prec[tmp]] = clk;
						tmpAns += ed[prec[tmp]].w;
					}
			}
...
void dfsS(int dep, int state)
{
    if (dep == sTot)
    {
        ...
        ++hasTot[kk][sTot];
        int ttt = hasTot[kk][sTot];
        REPP(j, 1, sTot)
			has[kk][sTot][ttt][j] = S[j];
		sort(has[kk][sTot][ttt] +1, has[kk][sTot][ttt] + sTot +1);
        if (state == (limS))
			dfsT(0, 0);
		return ;
	}
